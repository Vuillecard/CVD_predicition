...
def train_model(model, dataloaders, criterion, optimizer,device,path_save_model=None, num_epochs=25):
    since = time.time()
...
def accumulate_results(name_files,results_dir):
    ...
    perf_acc.pop('y_pred_proba',None)
    for i in range(1,len(name_files)):
        ...
        for metric in list(perf_acc.keys()) :
            perf_acc[metric] = np.c_[perf_acc[metric],perf_tmp[metric][:20]]
