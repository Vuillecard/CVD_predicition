...
def run(data_dir,annotated_file_dict, split_data, model_parameter, mode, samplers , which): 
    ...
    if which == 'augmented_culprit' : 
        ...
        image_datasets_train = {'train': CardioDataset(data_dir,annotations_img_id = split_data['train'],annotations_file_path = annotated_file_dict['train'] ,transform =True, to_tensor=True, norm=True,apply_attention = model_parameter['apply_attention'] ) ,
                                'val': CardioDataset(data_dir,annotations_img_id = split_data['val'], annotations_file_path = annotated_file_dict['val'] ,transform =False, to_tensor=True, norm=True,apply_attention = model_parameter['apply_attention'] ) }
        ...
        image_datasets_train = {x: CardioDataset(data_dir,annotations_img_id = split_data[x],annotations_file_path = annotated_file_dict[x],
...
def main_test(data_dir,results_save, model_parameter,num_runs,which,seed,modes):
    ...
    annotated_file_dict = { 'train' : annotated_file ,
                            'val': annotated_file_test}
    ...
    for mode in modes:
        ...
        for k in range(num_runs):
            perf = run(data_dir,annotated_file_dict,split_data,model_parameter,mode, samplers, which)
...
if __name__ == '__main__':
    ...
    if args.typerun == 'test_balance_channel' :
        print('Begin testing ...')
        num_runs = 5
        ...
        model_parameter['wd'] = 0.25
        model_parameter['dropout'] = False
        main_test(data_dir = os.path.join(data_path, 'data_CNN_ATT' ),results_save = os.path.join(results_dir, args.typerun ), 
                    model_parameter = model_parameter ,num_runs = num_runs,which = 'weighted_dataloader',seed = seed,modes=['ft_imgnet'])