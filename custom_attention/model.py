...
Modify for cardiovascular diseases prediction
"""
...
class Reduce_size(nn.Module):
    def __init__(self, in_channel=2, out_channel = 3 ):
        super(Reduce_size, self).__init__()
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, in_channel=3, num_classes=2, zero_init_residual=False):
...
class Large_input_ResNet(nn.Module):
    def __init__(self, in_channel = 2, num_classes=2,dropout = False,pretrained=False,load_best_model = False):
        super(Large_input_ResNet, self).__init__()

        self.reduce_input = Reduce_size(in_channel = in_channel)
        self.backbone = modelsT.resnet18(pretrained=pretrained)
...
class SupCEResNet(nn.Module):
    """encoder + classifier"""
    def __init__(self, name='resnet50', num_classes=10,pretrained = True):
        super(SupCEResNet, self).__init__()
        model_fun, dim_in = model_dict[name]
        if pretrained:
            state_dict = load_state_dict_from_url(model_urls[name], progress=True)
            model_fun.load_state_dict(state_dict)
        self.encoder = model_fun()
